...
async function  wasm_tester(circomInput, _options) {
    ...
    const folder = path.join("../build_circuits");
    const file = path.join("../build_circuits" + "/" + baseName + "_js/" + baseName + ".wasm");
    ...
    const wasm = await fs.promises.readFile(file);

    const wc = await WitnessCalculator(wasm);

    return new WasmTester(file, baseName, wc);
}
